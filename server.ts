    ...
    if (Array.isArray(orderData)) {
      ...
      const mergedOrders = globalOrders.map(existing => {
        if (incomingMap.has(existing.id)) {
          const updated = { ...existing, ...incomingMap.get(existing.id) };
      ...
      for (const targetOrder of newOrders) {
        if (targetOrder.userMemberId && globalUsersMap[targetOrder.userMemberId]) {
          const u = globalUsersMap[targetOrder.userMemberId];
          u.ordersCount = (u.ordersCount || 0) + 1;
          if (typeof targetOrder.coinsSpent === 'number' && targetOrder.coinsSpent > 0) {
            u.coins = Math.max(0, Math.round(((u.coins || 0) - targetOrder.coinsSpent) * 100) / 100);
            u.totalCoinsSpent = (u.totalCoinsSpent || 0) + targetOrder.coinsSpent;
            u.coinsUpdatedByAdmin = false;
          }
        }
      }
...
app.get('/api/users', (req, res) => {
  const now = Date.now();
  const ordersCountByMember: Record<string, number> = {};
  const coinsSpentByMember: Record<string, number> = {};

  globalOrders.forEach((o) => {
    if (o && o.userMemberId) {
      const cleanId = String(o.userMemberId).replace(/^#+/, '').trim();
      ordersCountByMember[cleanId] = (ordersCountByMember[cleanId] || 0) + 1;
      coinsSpentByMember[cleanId] = (coinsSpentByMember[cleanId] || 0) + (Number(o.coinsSpent) || 0);
    }
  });
  ...
  const usersList = Object.values(globalUsersMap).map((u: any) => {
    ...
    // Consider online if active heartbeat was received within last 120 seconds
    const isOnline = u.isOnline !== false && lastActiveTime > 0 && timeDiffMs < 120000 && timeDiffMs > -60000;
    ...
    const calculatedOrdersCount = ordersCountByMember[cleanMemberId] !== undefined
      ? Math.max(ordersCountByMember[cleanMemberId], u.ordersCount || 0)
      : (u.ordersCount || 0);
    const calculatedCoinsSpent = coinsSpentByMember[cleanMemberId] !== undefined
      ? Math.max(coinsSpentByMember[cleanMemberId], u.totalCoinsSpent || 0)
      : (u.totalCoinsSpent || 0);
...
// 5a. Get single user status check (for BlockedScreen Refresh Status)
app.get('/api/users/status/:memberId', (req, res) => {
  const memberId = String(req.params.memberId || '').trim();
  const user = globalUsersMap[memberId];
  if (user) {
    res.json({ success: true, memberId, status: user.status || 'ACTIVE' });
  } else {
    res.json({ success: false, memberId, status: 'NOT_FOUND' });
  }
});
...
app.post('/api/users/sync', (req, res) => {
  try {
    ...
    if (user && user.memberId) {
      ...
      // Server authoritative coins: if user provides explicit coins or user.coinsUpdatedByAdmin === false, use user.coins
      ...
      if (user.coinsUpdatedByAdmin === false) {
        coinsUpdatedByAdmin = false;
        if (typeof user.coins === 'number') {
          synchronizedCoins = user.coins;
        }
      } else if (coinsUpdatedByAdmin && typeof existing.coins === 'number') {
      ...
      globalUsersMap[user.memberId] = {
        ...existing,
        ...user,
        referralCode: user.referralCode || existing.referralCode || `ROX${user.memberId}`,
        name: existing.name || user.name || `User #${user.memberId}`,
...
app.post('/api/payment/submit', (req, res) => {
  try {
    ...
    const existingUtr = Object.values(globalPaymentRequestsMap).find((r: any) => r.utrNumber === cleanUtr);
    if (existingUtr) {
