...
export const AdminOrders: React.FC<AdminOrdersProps> = ({
  ...
  const handleClearFakeAndStoryOrders = () => {
    const validOnly = orders.filter((o) => {
      if (!o) return false;
      const orderId = String(o.id || '');
      const targetUrl = String(o.targetUrl || '');
      const svc = String(o.serviceType || '').toLowerCase();
      // Remove any fake simulated order or legacy Story Views orders
      return !orderId.startsWith('ord_fake_') && !targetUrl.includes('example.com') && !svc.includes('story');
    });
    onUpdateOrders(validOnly);
    showToast(`Cleaned test and Story Views orders. Remaining: ${validOnly.length}`);
  };
      ...
      <div className="bg-slate-900/90 border border-slate-800 p-5 rounded-2xl space-y-4">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            <button
              type="button"
              onClick={handleClearFakeAndStoryOrders}
              className="px-3 py-1.5 bg-slate-800 hover:bg-slate-700 text-amber-300 border border-slate-700 rounded-xl text-xs font-bold flex items-center gap-1.5 transition-all active:scale-95"
              title="Clean fake orders & remove Story Views orders"
            >
              <Trash2 className="w-3.5 h-3.5 text-amber-400" />
              <span>Clean Story/Test Orders</span>
            </button>

            <button
              type="button"
              onClick={handleClearAllOrders}
              className="px-2.5 py-1.5 bg-red-500/10 hover:bg-red-500/20 text-red-400 border border-red-500/30 rounded-xl text-xs font-bold transition-all"
