...
export const AdminOverview: React.FC<AdminOverviewProps> = ({
  ...
  const isUserOnline = (u: UserAccount) => {
    ...
    const lastActiveTime = typeof u.updatedAt === 'number' ? u.updatedAt : 0;
    if (!lastActiveTime) return u.isOnline === true;
    const timeDiffMs = Date.now() - lastActiveTime;
    return timeDiffMs < 75000 && timeDiffMs > -60000;
  };

  const activeOnlineUsers = users.filter(isUserOnline);
