...
import { getApiUrl, isUserOnline } from '../../utils/storage';
...
export const AdminUsers: React.FC<AdminUsersProps> = ({ users, orders, onUpdateUsers }) => {
  ...
  // Live auto-refresh ticker every 3s to re-calculate online presence against Date.now()
  React.useEffect(() => {
  ...
  const handleAdjustCoins = (userId: string, delta: number) => {
    ...
    const updated = users.map((u) => {
      if (u.id === userId || u.memberId === userId) {
        ...
        return { ...u, coins: newCoins, coinsUpdatedByAdmin: true, updatedAt: Date.now() };
      }
      return u;
    });
    ...
    if (targetUser) {
      const memberId = targetUser.memberId || targetUser.id;
      const newCoins = Math.max(0, (targetUser.coins || 0) + delta);
  ...
  const handleSetExactCoins = (userId: string, exactCoins: number) => {
    ...
    const updated = users.map((u) => {
      if (u.id === userId || u.memberId === userId) {
        return { ...u, coins: Math.max(0, exactCoins), coinsUpdatedByAdmin: true, updatedAt: Date.now() };
      }
      return u;
    });
    onUpdateUsers(updated);
  ...
  const handleToggleBlock = (userId: string) => {
    ...
    const updated = users.map((u) => {
      if (u.id === userId || u.memberId === userId) {
        newStatus = u.status === 'ACTIVE' ? ('BLOCKED' as const) : ('ACTIVE' as const);
    ...
    if (targetUser) {
      ...
      if (targetUser.authUid) {
        setDoc(doc(db, 'users_auth', targetUser.authUid), { 
          status: newStatus, 
          updatedAt: Date.now() 
        }, { merge: true }).catch(() => {});
      }
  ...
  const handleDeleteSingleUser = (u: UserAccount) => {
    const memberId = u.memberId || u.id;
    if (window.confirm(`Kya aap sach me Member #${memberId} (${u.name || 'User'}) ko permanently delete karna chahte hain?`)) {
      const updated = users.filter((item) => item.memberId !== memberId && item.id !== u.id);
      onUpdateUsers(updated);

      // 1. Delete on Server
      fetch(getApiUrl(`/api/users/${encodeURIComponent(memberId)}`), {
        method: 'DELETE'
      }).catch(() => {});

      // 2. Delete on Firestore
      if (memberId) {
        deleteDoc(doc(db, 'users', memberId)).catch(() => {});
      }

      showToast(`Member #${memberId} permanently deleted.`);
    }
  };
  ...
  const handleRemoveFakeUsers = () => {
    const realUsersOnly = users.filter((u) => {
      ...
      if (uId === 'usr_current') return true;
      const isMock =
  ...
  const onlineUsersCount = users.filter((u) => u && isUserOnline(u)).length;
  const offlineUsersCount = Math.max(0, users.length - onlineUsersCount);
      ...
      <div className="bg-slate-900/90 border border-slate-800 p-5 rounded-2xl space-y-4 shadow-xl">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            <div className="flex items-center gap-2">
              <h2 className="text-lg font-bold text-white flex items-center gap-2">
                <Users className="w-5 h-5 text-blue-400" /> Real-Time Global Users
              </h2>
            ...
            <p className="text-xs text-slate-400 mt-1">
              Worldwide real-time tracking: see active users online right now, device OS, country/location, and offline history.
            </p>
          </div>
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            {users.some((u) => u.id?.startsWith('usr_mock') || u.name === 'Rohan Sharma') && (
              <button
                ...
                title="Clear demo accounts and keep only real app users"
              >
                Clear Demo Data
              </button>
            )}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {filteredUsers.map((u) => {
          ...
          const userOrders = orders?.filter(o => 
            (u.memberId && o.userMemberId === u.memberId) || 
            (u.authUid && (o as any).authUid === u.authUid) ||
            (u.email && (o as any).userEmail === u.email)
          ) || [];
                ...
                <div className="flex items-center justify-between gap-1 flex-wrap">
                  ...
                  <div className="flex items-center gap-1.5">
                    ...
                    <button
                      ...
                      className="px-2 py-1 text-[11px] font-bold rounded-lg transition-colors flex items-center gap-1 bg-red-500/10 hover:bg-red-500/20 text-red-400 border border-red-500/20"
                      title="Delete User"
                    >
