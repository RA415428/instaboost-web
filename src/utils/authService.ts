...
export async function handleFirebaseAuthUser(
  ...
  // 1. Check if user already exists in Firestore by authUid or email
  let existingMemberId: string | null = null;
  let existingUserData: any = null;

  // Check direct user doc by authUid
  try {
    const authUserSnap = await getDoc(doc(db, 'users_auth', authUid));
  ...
  // If not found in mapping, query users collection
  if (!existingMemberId) {
    try {
      const usersSnap = await getDocs(collection(db, 'users'));
      usersSnap.forEach((docSnap) => {
        const data = docSnap.data();
        if (data.authUid === authUid || (email && data.email && data.email.toLowerCase() === email.toLowerCase())) {
          existingMemberId = data.memberId || docSnap.id;
    ...
    try {
      const userDocSnap = await getDoc(doc(db, 'users', existingMemberId));
  ...
  if (existingMemberId && existingUserData) {
    ...
    // Restore user orders from Cloud Firestore
    try {
      const ordersSnap = await getDocs(collection(db, 'orders'));
      const restoredOrders: Order[] = [];
      ordersSnap.forEach((docSnap) => {
        const o = docSnap.data() as Order;
        if (o.userMemberId === existingMemberId || (o as any).authUid === authUid) {
          restoredOrders.push(o);
        }
      });
      ...
      console.warn('Orders restore on Google login error:', err);
    }
...
export async function handleFirestoreEmailSignIn(
  ...
  try {
    ...
    if (!accountDoc.exists() && isOwnerUser) {
    ...
    let existingUserData = userSnap.exists() ? userSnap.data() : null;
...
export async function signInWithGoogle(customWelcomeBonus?: number): Promise<GoogleAuthResult> {
  ...
  try {
    ...
    // If popup was blocked or not supported by WebView, attempt redirect flow
    if (
      ...
      try {
        console.log('Attempting signInWithRedirect fallback for WebView...');
        await signInWithRedirect(auth, googleProvider);
        return {
          success: false,
          error: 'Redirecting to Google Sign-In...'
