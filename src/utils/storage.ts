import { ActivityLog, AdminConfig, Order, PaymentRequest, UserAccount, UserWallet } from '../types';
import { DEFAULT_ADMIN_CONFIG } from './defaultAdminConfig';
import { initialOrders } from '../data/appData';
import { db } from '../lib/firebase';
import { doc, getDoc, setDoc, deleteDoc, getDocs, collection, query, orderBy, limit, onSnapshot, runTransaction } from 'firebase/firestore';
...
export function getApiUrl(path: string): string {
  ...
  // 1. Check for custom backend URL saved in localStorage or env
  try {
  ...
  // Production Central Cloud Server URL fallback for standalone APKs and Firebase Hosting
  const CENTRAL_SERVER_URL = 'https://ais-pre-l72l5hitdvz2jryudn72qo-609197389972.asia-southeast1.run.app';

  if (typeof window !== 'undefined' && window.location && window.location.origin) {
    const origin = window.location.origin;

    // Detect static hosting (Firebase Hosting, GitHub Pages, Vercel, Netlify) or local WebView files
    ...
    // Only route locally if running directly on the Node.js Express server container
    if (!isStaticHosting && (origin.startsWith('http://') || origin.startsWith('https://'))) {
      return `${origin}${cleanPath}`;
    }
  }

  return `${CENTRAL_SERVER_URL}${cleanPath}`;
}
...
export function saveOrders(orders: Order[]): void {
  try {
    // 1. Read previous orders from localStorage to compare and find changes
    let oldOrders: Order[] = [];
    try {
      const cached = localStorage.getItem(ORDERS_KEY);
      if (cached) {
        oldOrders = JSON.parse(cached);
      }
    } catch (_) {}

    // 2. Perform the update locally in LocalStorage & dispatch event
    localStorage.setItem(ORDERS_KEY, JSON.stringify(orders));
    window.dispatchEvent(new Event('instaboost_orders_updated'));

    // 3. Sync full list to Express Server (it needs the complete list as backend source-of-truth)
    ...
    // 4. Determine Firestore updates/deletes to avoid overwriting other users' orders or reverting completed orders
    if (Array.isArray(orders)) {
      const oldOrdersMap = new Map((oldOrders || []).map((o) => [o?.id, o]));
      const newOrdersMap = new Map(orders.map((o) => [o?.id, o]));

      // Deleted orders: exist in oldOrders but not in newOrders
      const deletedOrders = (oldOrders || []).filter((o) => o && o.id && !newOrdersMap.has(o.id));

      // New or modified orders: exist in newOrders but not in oldOrders OR have different status/SMM details
      const changedOrNewOrders = orders.filter((o) => {
        if (!o || !o.id) return false;
        const oldO = oldOrdersMap.get(o.id);
        if (!oldO) return true; // Brand new order
        return (
          oldO.status !== o.status ||
          oldO.smmOrderId !== o.smmOrderId ||
          oldO.smmResponse !== o.smmResponse
        );
      });
      ...
      deletedOrders.forEach((o) => {
        if (o.id) {
          deleteDoc(doc(db, 'orders', o.id)).catch((err) =>
            console.error(`Failed to delete order ${o.id} from Firestore:`, err)
          );
        }
      });

      // Sync modifications/additions to Firestore
      changedOrNewOrders.forEach((o) => {
        if (o.id) {
          setDoc(doc(db, 'orders', o.id), o, { merge: true }).catch((err) =>
            console.error(`Failed to set order ${o.id} in Firestore:`, err)
          );
        }
      });
    }
...
export async function fetchServerAdminConfig(): Promise<AdminConfig | null> {
  ...
  try {
    ...
    if (docSnap.exists()) {
      ...
      if (firestoreConfig && (firestoreConfig.lastUpdated || 0) >= (localConfig.lastUpdated || 0)) {
        const merged: AdminConfig = {
          ...localConfig,
          ...firestoreConfig,
          pricing: { ...localConfig.pricing, ...(firestoreConfig.pricing || {}) },
          ads: { ...localConfig.ads, ...(firestoreConfig.ads || {}) },
          appUpdate: { ...(DEFAULT_ADMIN_CONFIG.appUpdate!), ...(localConfig.appUpdate || {}), ...(firestoreConfig.appUpdate || {}) },
          coinPackages: firestoreConfig.coinPackages?.length ? firestoreConfig.coinPackages : localConfig.coinPackages,
          subscriptionPackage: firestoreConfig.subscriptionPackage || localConfig.subscriptionPackage
        };
  ...
  try {
    ...
    if (res.ok) {
      ...
      if (data && data.success && data.config) {
        ...
        if (serverConfig && (serverConfig.lastUpdated || 0) >= (localConfig.lastUpdated || 0)) {
          const merged: AdminConfig = {
            ...localConfig,
            ...serverConfig,
  ...
  try {
    ...
    if (!querySnap.empty) {
      ...
      if (firestoreOrders.length > 0) {
        localStorage.setItem(ORDERS_KEY, JSON.stringify(firestoreOrders));
        window.dispatchEvent(new Event('instaboost_orders_updated'));
  ...
  try {
    ...
    if (!querySnap.empty) {
      ...
      querySnap.forEach((docSnap) => {
        ...
        const lastActiveTime = typeof u.lastSeenAt === 'number' && u.lastSeenAt > 0
          ? u.lastSeenAt
          : (typeof u.updatedAt === 'number' && u.updatedAt > 0 ? u.updatedAt : 0);
        const timeDiffMs = now - lastActiveTime;
        const isOnline = u.isOnline !== false && lastActiveTime > 0 && timeDiffMs < 75000 && timeDiffMs > -60000;
        ...
        firestoreUsers.push({
          ...u,
          isOnline,
          updatedAt: u.updatedAt || lastActiveTime,
          lastSeenAt: lastActiveTime,
...
export async function initAutoDeviceRestore(): Promise<{ restored: boolean; memberId?: string; coins?: number }> {
  try {
    ...
    const deviceSnap = await withTimeout(getDoc(deviceRef), 3500);
    ...
    // 2. Fallback query if device mapping not directly present
    if (!restoredMemberId) {
      const querySnap = await withTimeout(getDocs(collection(db, 'users')), 3500);
      if (!querySnap.empty) {
        querySnap.forEach((docSnap) => {
          const data = docSnap.data();
          if (data.deviceFingerprint === fp && data.memberId) {
    ...
    if (restoredMemberId) {
      ...
      // Restore orders from Firestore
      try {
        const ordersSnap = await withTimeout(getDocs(collection(db, 'orders')), 3500);
        if (!ordersSnap.empty) {
          const restoredOrders: Order[] = [];
          ordersSnap.forEach((docSnap) => {
            const oData = docSnap.data() as Order;
            if (oData.userMemberId === restoredMemberId) {
        ...
        console.warn('Orders restore warning:', e);
      }
    ...
    console.warn('Auto device restore error:', err);
  }
  return { restored: false };
}
...
export function isUserOnline(u: UserAccount): boolean {
  ...
  if (u.isOnline === false) {
    const lastActiveTime = Math.max(
      typeof (u as any).lastSeenAt === 'number' ? (u as any).lastSeenAt : 0,
      typeof u.updatedAt === 'number' ? u.updatedAt : 0
    );
    if (!lastActiveTime || Date.now() - lastActiveTime > 45000) {
      return false;
    }
  }
  const lastActiveTime = Math.max(
  ...
  if (!lastActiveTime) return Boolean(u.isOnline);
  const timeDiffMs = Date.now() - lastActiveTime;
  return timeDiffMs < 120000 && timeDiffMs > -60000;
}
...
export async function syncUserDeviceWithServer(passedWallet?: UserWallet): Promise<void> {
  ...
  try {
    ...
    const fp = getDeviceFingerprint();
    const userRef = doc(db, 'users', wallet.memberId);
    const existingSnap = await withTimeout(getDoc(userRef), 2500).catch(() => null);
    
    // Always preserve local authoritative coin balance unless remote server/admin updated it
    let activeCoins = typeof wallet.coins === 'number' ? wallet.coins : 0;
    let activeAdsWatched = typeof wallet.dailyAdsWatched === 'number' ? wallet.dailyAdsWatched : 0;
    let activeMaxDailyAds = wallet.maxDailyAds || 10;
    let activeStatus = 'ACTIVE';
    let activeName = `User #${wallet.memberId}`;
    let coinsUpdatedByAdmin: boolean = false;
    ...
    if (existingSnap && existingSnap.exists()) {
      ...
      if (isExplicitLocalUpdate) {
        // Local wallet was explicitly changed by user action (earned ad coins or placed order)
        activeCoins = typeof wallet.coins === 'number' ? wallet.coins : 0;
        activeAdsWatched = typeof wallet.dailyAdsWatched === 'number' ? wallet.dailyAdsWatched : 0;
        coinsUpdatedByAdmin = false;
      } else {
        // Passive heartbeat: only adopt remote balance if admin explicitly modified it AND the remote change is newer than our local coin transaction
        const remoteUpdatedTime = existingData.updatedAt || existingData.lastSeenAt || 0;
        if (
          existingData.coinsUpdatedByAdmin === true &&
          existingData.coins !== undefined &&
          remoteUpdatedTime > localMutationTime
        ) {
          const remoteCoins = Number(existingData.coins);
          if (!isNaN(remoteCoins)) {
            activeCoins = remoteCoins;
            wallet.coins = activeCoins;
            wallet.lastLocalCoinMutationAt = remoteUpdatedTime;
      ...
      if (existingData.maxDailyAds !== undefined) {
        activeMaxDailyAds = Number(existingData.maxDailyAds);
        wallet.maxDailyAds = activeMaxDailyAds;
        saveUserWallet(wallet);
      }
      if (existingData.status) {
        activeStatus = existingData.status;
      }
      if (existingData.name) {
        activeName = existingData.name;
      }
    }
    ...
    const userPayload: any = {
      id: `usr_${wallet.memberId}`,
      userId: `#${wallet.memberId}`,
      memberId: wallet.memberId,
      deviceFingerprint: fp,
      name: activeName,
      coins: activeCoins,
      dailyAdsWatched: activeAdsWatched,
      maxDailyAds: activeMaxDailyAds,
      ...
      updatedAt: Date.now(),
      lastSeenAt: Date.now(),
      lastLocalCoinMutationAt: wallet.lastLocalCoinMutationAt || Date.now(),
      createdAt: existingSnap?.exists() && existingSnap.data()?.createdAt ? existingSnap.data().createdAt : Date.now(),
      deviceType,
      location,
      status: activeStatus,
      coinsUpdatedByAdmin: Boolean(coinsUpdatedByAdmin)
    };

    if (existingSnap && existingSnap.exists()) {
      await setDoc(userRef, { ...existingSnap.data(), ...userPayload, coinsUpdatedByAdmin: Boolean(coinsUpdatedByAdmin) }, { merge: true });
    } else {
      await setDoc(userRef, userPayload, { merge: true });
    }

    // Save hardware device mapping document in Firestore
    setDoc(doc(db, 'devices', fp), { memberId: wallet.memberId, updatedAt: Date.now() }, { merge: true }).catch(() => {});
    ...
    console.warn('Firestore user heartbeat sync error:', e);
  }
}
...
export function subscribeToFirestore(
  ...
  const unsubConfig = onSnapshot(doc(db, 'config', 'global'), (docSnap) => {
    if (docSnap.exists()) {
      ...
      if (data) {
        ...
        if ((data.lastUpdated || 0) < (local.lastUpdated || 0)) {
          return;
        }
  ...
  const unsubUsers = onSnapshot(collection(db, 'users'), (querySnap) => {
    ...
    querySnap.forEach((docSnap) => {
      ...
      const lastActiveTime = typeof u.lastSeenAt === 'number' && u.lastSeenAt > 0
        ...
        : (typeof u.updatedAt === 'number' && u.updatedAt > 0 ? u.updatedAt : 0);
      const timeDiffMs = now - lastActiveTime;
      // Mark online if active within last 75 seconds (or up to 60s future clock skew)
      const isOnline = u.isOnline !== false && lastActiveTime > 0 && timeDiffMs < 75000 && timeDiffMs > -60000;
      ...
      if (u.memberId === currentMemberId && u.status) {
  ...
  if (currentMemberId) {
    unsubUser = onSnapshot(doc(db, 'users', currentMemberId), (docSnap) => {
      if (docSnap.exists()) {
    ...
    }, (err) => console.warn('Firestore current user listener error:', err));
  }
...
export async function checkUserBlockedStatus(memberId: string): Promise<{ isBlocked: boolean; status: 'ACTIVE' | 'BLOCKED' }> {
  try {
    ...
    try {
      ...
      console.warn('Firestore direct user status check error:', e);
    }
    ...
    try {
      ...
      console.warn('Server status check error:', e);
    }
  } catch (err) {
    console.warn('checkUserBlockedStatus general error:', err);
  }
...
export function saveUsersList(users: UserAccount[]): void {
  try {
    ...
    // Direct Firestore Sync for each user document
    if (Array.isArray(users)) {
      users.forEach((u) => {
        if (u.memberId) {
          const docData: any = {
            id: u.id || `usr_${u.memberId}`,
            memberId: u.memberId,
            name: u.name,
          ...
          if (typeof u.lastLocalCoinMutationAt === 'number') {
            docData.lastLocalCoinMutationAt = u.lastLocalCoinMutationAt;
          }
          if (typeof u.lastSeenAt === 'number' && u.lastSeenAt > 0) {
            docData.lastSeenAt = u.lastSeenAt;
          }
          if (typeof u.updatedAt === 'number' && u.updatedAt > 0) {
            docData.updatedAt = u.updatedAt;
          }
          if (u.isOnline !== undefined) {
            docData.isOnline = u.isOnline;
          }
          if (u.lastActive) {
            docData.lastActive = u.lastActive;
          }
          setDoc(doc(db, 'users', u.memberId), docData, { merge: true }).catch(() => {});
        }
      });
    }
  } catch (err) {
...
export async function fetchUserReferralHistory(memberId: string): Promise<{
  ...
  try {
    const res = await fetch(getApiUrl(`/api/referral/my-history/${cleanId}`), { cache: 'no-store' });
    if (res.ok) {
      const data = await res.json();
    ...
    console.warn('Failed to fetch referral history from server:', e);
  }
...
export async function submitPaymentRequest(payload: {
  ...
  try {
    const cleanUtr = payload.utrNumber.toString().trim();
    ...
    // 1. Direct write to Firebase Firestore (Guarantees persistence even on slow network)
    ...
    // 2. If auto approved, credit coins in Firestore and local wallet immediately
    if (autoApproved) {
    ...
    // 3. Asynchronously sync to backend Express server
    fetch(getApiUrl('/api/payment/submit'), {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ ...payload, paymentRecord: payRecord })
    }).catch(() => {});
...
export function subscribeToPaymentRequests(onRequestsUpdate: (requests: PaymentRequest[]) => void) {
  ...
  }, (err) => console.warn('Payment requests listener error:', err));
}
